...
export default function ParentMessagesPage() {
  ...
  const getThreadDisplay = (thread: MessageThread) => {
    ...
    return {
      ...
      role: counterpart?.title ?? 'Teacher',
    }
  }
  ...
  const handleCreateThread = () => {
    ...
    const newEntry: MessageThread = {
      ...
      participants: [
        {
          role,
          name: profile.name || session.name || 'Parent',
          title: profile.role || 'Parent',
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Chat with teachers and keep communication organized.
          </p>
        </div>
        <button
        ...
        <div className="card xl:col-span-2">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between mb-4">
            <div>
              ...
              {activeThread && (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {activeDisplay?.role ?? 'Teacher'} - {activeThread.status}
                </p>
              )}
            </div>
            <button
