...
import { ArrowLeft, User, Mail, Phone, Book, School, Lock, Check } from 'lucide-react'
...
export default function TeacherRegistrationPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    employeeId: '', joinDate: '', employmentType: '',
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      document.cookie = `user_type=teacher; path=/;`
      try {
        ...
        localStorage.setItem('esm_profile', JSON.stringify({
          name: user.name, role: 'Teacher', email: user.email,
          phone: formData.phone, department: formData.department, subjects: formData.subjects,
        }))
      } catch { }
      router.push(`/auth/register/success?type=teacher&email=${encodeURIComponent(formData.email)}`)
    } catch (err: any) {
  ...
  const steps = [
    ...
    { number: 2, title: 'Professional', icon: Book },
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-950 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Teacher Registration</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {step === 2 && (
            <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div><label className="block text-sm font-medium mb-2">Department *</label><input type="text" name="department" value={formData.department} onChange={handleChange} required className="input-field" placeholder="e.g. Mathematics" /></div>
                <div><label className="block text-sm font-medium mb-2">Subjects *</label><input type="text" name="subjects" value={formData.subjects} onChange={handleChange} required className="input-field" placeholder="e.g. Math, Physics" /></div>
                <div><label className="block text-sm font-medium mb-2">Qualification</label><input type="text" name="qualification" value={formData.qualification} onChange={handleChange} className="input-field" placeholder="e.g. B.Ed, M.Sc" /></div>
                <div><label className="block text-sm font-medium mb-2">Years of Experience</label><input type="number" name="experience" value={formData.experience} onChange={handleChange} min="0" className="input-field" placeholder="Years" /></div>
                <div><label className="block text-sm font-medium mb-2">Employment Type</label>
                ...
                <div><label className="block text-sm font-medium mb-2">Join Date</label><input type="date" name="joinDate" value={formData.joinDate} onChange={handleChange} className="input-field" /></div>
              </div>
            </div>
          )}
            ...
            <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
              ...
              <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div><p className="text-gray-500">Department</p><p className="font-medium">{formData.department || 'Not provided'}</p></div>
                  <div><p className="text-gray-500">Subjects</p><p className="font-medium">{formData.subjects || 'Not provided'}</p></div>
                </div>
              </div>
            </div>
          )}
