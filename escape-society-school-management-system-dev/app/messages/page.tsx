...
export default function MessagesPage() {
  ...
  const inactiveThreads = useMemo(
    () => visibleThreads.filter((thread) => (messagesByThread[thread.id] || []).length === 0).length,
    [messagesByThread, visibleThreads]
  )
  ...
  const handleCreateThread = () => {
    ...
    const newEntry: MessageThread = {
      ...
      participants: [
        {
          role,
          name: profile.name || session.name || 'Admin',
          title: profile.role || 'Administrator',
    ...
    <DashboardShell>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              Keep track of conversations across staff, teachers, and families.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">Active Threads</h3>
          ...
          <div className="card xl:col-span-2">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between mb-4">
              <div>
                ...
                {activeThread && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {activeDisplay?.role ?? 'School'} - {activeThread.status}
                  </p>
                )}
              </div>
              <button
