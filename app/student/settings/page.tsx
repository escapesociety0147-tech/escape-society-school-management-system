...
const initialStudentNotificationPrefs = [
...
export default function StudentSettingsPage() {
  const [profile, setProfile] = useLocalStorageState('esm_student_profile', initialStudentProfile)
  const [preferences, setPreferences] = useLocalStorageState('esm_student_preferences', {
    language: 'English (US)',
    timezone: 'America/Los_Angeles',
    dateFormat: 'MM/DD/YYYY',
    defaultDashboard: 'Student Overview',
  })
  const [notificationPrefs, setNotificationPrefs] = useLocalStorageState(
    'esm_student_notification_prefs',
    initialStudentNotificationPrefs
  )
  ...
  const passwordUpdatedLabel = studentUser?.createdAt
    ? new Date(studentUser.createdAt).toLocaleDateString('en-US', {
        month: 'short',
        day: '2-digit',
        year: 'numeric',
      })
  ...
  const attendanceCount = useMemo(() => {
    return attendanceHistory.filter(
      (record) =>
        ...
        record.attendance?.[profile.id]
    ).length
  }, [attendanceHistory, profile.grade, profile.section, profile.id])
  const resultCount = results.filter((result) => result.rollNo === profile.rollNumber).length
  const paymentCount = payments.filter((payment) => payment.rollNo === profile.rollNumber).length
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setProfile((prev) => ({ ...prev, rollNumber: event.target.value }))
                  }
                  className="input-field mt-1"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  value={profile.grade}
                  onChange={(event) =>
                    setProfile((prev) => ({ ...prev, grade: event.target.value }))
                  }
                  className="input-field mt-1"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setProfile((prev) => ({ ...prev, section: event.target.value }))
                  }
                  className="input-field mt-1"
                />
              </div>
          ...
          <div className="card">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  <option>Student Overview</option>
